...
# MAGIC %md
# MAGIC # generate_levenshtein_test_data_1m — Insert ~1M levenshtein-test rows into source_motor_devtest
# MAGIC
# MAGIC **DATA_DT:** `2025-02-03`
# MAGIC
# MAGIC | Target | Rows |
# MAGIC |---|---|
# MAGIC | `source_motor_devtest` | ~1,000,000 |
# MAGIC | `trust_source_devtest` | copy from `DATA_DT='2025-01-01'` |
# MAGIC
# MAGIC **ID card distribution:**
# MAGIC - 200,000 rows: unique id_card (prefix `'6'`, singletons)
# MAGIC - ~800,000 rows: 200,000 id_card groups (prefix `'7'`), 3–5 policies each
# MAGIC
# MAGIC **Strategy:** Build rows in Python batches of 50K then insert per batch to avoid driver OOM.
# MAGIC
# MAGIC **Idempotent:** Step 0 deletes existing `2025-02-03` partition before inserting.
...
LEV_DATA_DT  = "2025-02-03"

N_UNIQUE     = 200_000   # singleton rows (unique id_card, prefix '6')
N_GROUPS     = 200_000   # shared id_card groups (prefix '7'), 3-5 policies each
...
def make_variants(fname, lname, n):
    variants = [(fname, lname)]
    if n >= 2: variants.append((fname, lname + "ๆ"))
    if n >= 3: variants.append((fname[:-1], lname))
    if n >= 4: variants.append((fname, lname[:-1] + "า"))
    if n >= 5: variants.append(("ก" + fname, lname))
    return variants[:n]
...
def insert_batch(batch, batch_num):
    ...
    df.createOrReplaceTempView("lev_batch")
    spark.sql(f"""
...
# MAGIC %md
# MAGIC ## Step 2: Insert singleton rows in batches (200K rows)
...
# MAGIC %md
# MAGIC ## Step 3: Insert group rows in batches (~800K rows)

# COMMAND ----------

print(f"=== Inserting group rows ({N_GROUPS:,} groups) ===")
batch = []
batch_num = 0
...
for g in range(N_GROUPS):
    ...
    for fname, lname in variants:
        batch.append(make_row(
            f"lev3{str(policy_counter).zfill(7)}",
            id_card,
            fname,
            lname,
            policy_counter,
...
print(f"trust_source_devtest [{LEV_DATA_DT}] : {trust_count:,} rows")
...
group_id_count = spark.sql(f"SELECT COUNT(DISTINCT id_card) AS n FROM {MOTOR_TABLE} WHERE DATA_DT = '{LEV_DATA_DT}' AND id_card LIKE '7%'").collect()[0]["n"]
unique_id_count = spark.sql(f"SELECT COUNT(DISTINCT id_card) AS n FROM {MOTOR_TABLE} WHERE DATA_DT = '{LEV_DATA_DT}' AND id_card LIKE '6%'").collect()[0]["n"]

print("=== Verification ===")
print(f"source_motor_devtest [{LEV_DATA_DT}] : {motor_count:,} rows")
