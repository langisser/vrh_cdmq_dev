...
# MAGIC %md
# MAGIC # generate_levenshtein_test_data_100k — Insert ~100K levenshtein-test rows into source_motor_devtest
# MAGIC
# MAGIC **DATA_DT:** `2025-02-02`
# MAGIC
# MAGIC | Target | Rows |
# MAGIC |---|---|
...
# MAGIC - 20,000 rows: unique id_card (prefix `'6'`, singletons)
# MAGIC - ~80,000 rows: 20,000 id_card groups (prefix `'7'`), 3–5 policies each
# MAGIC   - Each group has a base fname/lname + levenshtein variants (distance 1)
# MAGIC   - Exercises matching rules 32/33/35/36/38/39
# MAGIC
# MAGIC **Idempotent:** Step 0 deletes existing `2025-02-02` partition before inserting.
...
LEV_DATA_DT  = "2025-02-02"

N_UNIQUE     = 20_000   # singleton rows (unique id_card, prefix '6')
N_GROUPS     = 20_000   # shared id_card groups (prefix '7'), 3-5 policies each
...
# Build singleton rows
singleton_rows = []
for i in range(N_UNIQUE):
    prov = PROVINCES[i % len(PROVINCES)]
    ts   = make_ts(i)
    singleton_rows.append((
        f"lev2{str(i).zfill(6)}",
        "6" + str(i).zfill(12),
        FNAMES[i % len(FNAMES)],
        LNAMES[i % len(LNAMES)],
        ...
        f"08{str(i % 100_000_000).zfill(8)}" if i % 10 < 3 else None,
        ts, ts,
        LEV_DATA_DT,
    ))
...
for g in range(N_GROUPS):
    ...
    for v, (fname, lname) in enumerate(variants):
        prov = PROVINCES[policy_counter % len(PROVINCES)]
        ts   = make_ts(policy_counter)
        group_rows.append((
            f"lev2{str(policy_counter).zfill(6)}",
            id_card,
            fname,
            lname,
            GENDERS[policy_counter % len(GENDERS)],
            TABLES[policy_counter % len(TABLES)],
            None,
            PREFIXES[policy_counter % len(PREFIXES)],
            prov[0], prov[1], prov[2], prov[3],
            f"user{policy_counter}@example.com" if policy_counter % 5 == 0 else None,
            f"08{str(policy_counter % 100_000_000).zfill(8)}" if policy_counter % 10 < 3 else None,
            ts, ts,
            LEV_DATA_DT,
        ))
        policy_counter += 1
...
df.createOrReplaceTempView("lev_data_100k")
...
group_id_count = spark.sql(f"""
    SELECT COUNT(DISTINCT id_card) AS n FROM {MOTOR_TABLE}
...
# MAGIC %md
# MAGIC ## Step 3: Copy trust_source rows for DATA_DT='2025-02-02'
