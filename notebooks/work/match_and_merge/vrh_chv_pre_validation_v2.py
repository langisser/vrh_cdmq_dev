...
# MAGIC
# MAGIC #### Sample PARAMS:
# MAGIC viriyah_cdqm_poc.silver.SOURCE_MOTOR^|viriyah_cdqm_poc.control_fw.chv_pre_validation_result_v2^|2026-01-07^|SOURCE_MOTOR^|2^|SOURCE_MOTOR^|2
# MAGIC
# MAGIC #### Changes from v1
# MAGIC - All config table references use _v2 suffix:
# MAGIC   CHV_CONFIG_PK_V2, CHV_CONFIG_PRE_VALIDATION_V2, CHV_CONFIG_FUNCTION_V2, CHV_PARAM_GENERAL_V2
# MAGIC - Result table is passed as parameter (chv_pre_validation_result_v2 from main_v2)

# COMMAND ----------

# viriyah_cdqm_poc.silver.SOURCE_MOTOR^|viriyah_cdqm_poc.control_fw.chv_pre_validation_result_v2^|2026-01-07^|SOURCE_MOTOR^|2^|SOURCE_MOTOR^|2
...
# All config queries use _v2 tables
config_pk = spark.sql(
    f"select * from {catalog}.{fw_schema}.CHV_CONFIG_PK_V2 WHERE ACT_F = '1' ORDER BY ORDER"
    .replace('%ENV%', env)
)
config_validation = spark.sql(f"""
    select config_val.*, config_function.FUNCTION, config_function.PROGRAMMING
    from {catalog}.{fw_schema}.CHV_CONFIG_PRE_VALIDATION_V2 config_val
    left join {catalog}.{fw_schema}.CHV_CONFIG_FUNCTION_V2 config_function
...
if len(main) != 0:
  ...
  sql = f"""SELECT {",".join( [i for i in parameter])}, {pk} \n FROM {main_table} main \n LEFT JOIN {catalog}.{fw_schema}.CHV_PARAM_GENERAL_V2 PARM ON LOWER(PARM.PARAM_NAME) = '{table_nm.lower()}' AND UPPER(PARM.PARAM_GROUP_NAME) = 'DATE' WHERE main.DATA_DT = DATE_FORMAT(DATE_ADD(TO_DATE('%DATA_DT%','yyyy-MM-dd'),{general_param.filter(lower(col('PARAM_NAME')) == table_nm.lower()).collect()[0].PARAM_VAL_NUMBER}),'yyyy-MM-dd')"""
...
# Write to chv_pre_validation_result_v2 (vld_table is set by main_v2 to the _v2 table)
