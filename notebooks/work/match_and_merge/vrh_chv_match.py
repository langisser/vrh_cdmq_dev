...
# MAGIC %md
# MAGIC ##Generate BKEY
# MAGIC Generate a BKEY after passing post-validation for customers who are identified as identical.
# MAGIC

# COMMAND ----------

#Get max BKEY from table viriyah_cdqm_poc.silver.chv_table_bkey
bkey = int(spark.sql(f"select COALESCE(max(BKEY),0) as BKEY from {catalog}.{data_schema}.chv_table_bkey".replace('%ENV%',env)).collect()[0].BKEY)
print(bkey)
...
# MAGIC %md
# MAGIC ##Generate BKEY that exists in table bkey
# MAGIC
# MAGIC   
...
bkey_exists_rs = bkey_exists.select(col('KEY_MAIN').alias('KEY'),col('MAIN_TABLE').alias('TABLE'),col('MATCH_BKEY').alias('BKEY'))
...
# MAGIC %md
# MAGIC ##Generate  BKEY for a key that successfully passes post-validation and does not exists BKEY in the BKEY table.
...
bkey_df = pd.DataFrame({'KEY':KEY,'TABLE':TABLE,'BKEY':BKEY})
...
# MAGIC %md
# MAGIC ##Generate  BKEY for a key that not passes post-validation and does not exists BKEY in the BKEY table.

# COMMAND ----------

not_pass_post = not_pass_post.withColumn("BKEY", lit(None))
window_spec = Window.orderBy("BKEY")  # Replace "any_column" with the appropriate column for ordering

# Generate a running number
not_pass_post = not_pass_post.withColumn("running_number", row_number().over(window_spec))
not_pass_post = not_pass_post.withColumn('BKEY',col('running_number')+bkey).select(col('DATA_KEY').alias('KEY'),lit(table.replace('%ENV%',env)).alias("TABLE"),col('BKEY'))
